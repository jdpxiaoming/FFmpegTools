...
#include <stdbit.h>
#else
// Compatibility implementation for stdbit.h functions when C23 is not available
static inline unsigned int stdc_count_ones(unsigned int value)
{
...
static inline unsigned int stdc_trailing_zeros(unsigned int value)
{
#if defined(__GNUC__) || defined(__clang__)
    return value ? __builtin_ctz(value) : sizeof(value) * 8;
#else
    // Manual implementation
    if (value == 0)
