...
#if CONFIG_AVDEVICE
    // Special handling for avdevice: use version macro instead of function
    // to avoid linking issues when libffmpeg.so doesn't export avdevice_version
    if (CONFIG_AVDEVICE) {
        const char *indent = flags & INDENT? "  " : "";
        if (flags & SHOW_VERSION) {
            unsigned int version = LIBAVDEVICE_VERSION_INT;
            av_log(NULL, level,
                   "%slib%-11s %2d.%3d.%3d / %2d.%3d.%3d\n",
                   indent, "avdevice",
        ...
        if (flags & SHOW_CONFIG) {
            // Skip config check for avdevice to avoid linking issues
        }
    }
#else
